    ...
    while(!WindowShouldClose()){
        ...
        if(paddle_A_points == WIN_LIMIT || paddle_B_points == WIN_LIMIT){
            ...
            if(paddle_A_points > paddle_B_points){
                DrawText(TextFormat("PLAYER A WON!"), 0.16f * WIDTH, 0.434f * HEIGHT, FONT_SIZE, FG_COLOR);
            }
            if(paddle_B_points > paddle_A_points){
                DrawText(TextFormat("PLAYER B WON!"), 0.16f * WIDTH, 0.434f * HEIGHT, FONT_SIZE, FG_COLOR);
            }
        }
